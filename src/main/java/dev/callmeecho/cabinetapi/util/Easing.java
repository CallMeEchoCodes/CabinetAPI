...
public enum Easing {
     ...
     * @see #QUINT
     */
    public static Double2DoubleFunction polynomial(int power) { return t -> Math.pow(t, power); }

    /**
     * @see #BACK
     */
    public static Double2DoubleFunction back(double s) { return t -> t * t * ((s + 1) * t - s); }
    ...
    Easing(Double2DoubleFunction function) { this.function = function; }

    public double in(double t) { return function.get(t); }
    public double out(double t) { return 1 - function.get(1 - t); }
    public double inOut(double t) { return t < 0.5F ? in(t * 2) / 2 : 1 - in((1 - t) * 2) / 2; }

    public double in(double t, double start, double end, double duration) { return start + (end - start) * in(t / duration); }
    public double out(double t, double start, double end, double duration) { return start + (end - start) * out(t / duration); }
    public double inOut(double t, double start, double end, double duration) { return start + (end - start) * inOut(t / duration); }
